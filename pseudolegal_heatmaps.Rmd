...
# Legal and Pseudolegal Documents Heat maps

## Setup 
This RMD has all the code for creating the heat maps. 
...
# gets unique features, counts occurrences, drops any under 10
...
This code loops through the list of features that were marked / coded originally. Then it calculates a probability density estimation, in this case a kernel density estimation of the spatial dimensions of the feature on the page.
...
```{r single_histogram}
LLD_feature <- LLD %>% filter(feature == "footer")
LLD_feature_coord <- LLD_feature %>% dplyr::select(x, y) %>% as.data.frame()
density_LLD <- kde2d(LLD_feature_coord$x, LLD_feature_coord$y)
density_LLD_df <- kde2d(LLD_feature_coord$x, LLD_feature_coord$y) %>% as.data.frame()

SCD_feature <- SCD %>% filter(feature == "footer")
SCD_feature_coord <- SCD_feature %>% dplyr::select(x, y) %>% as.data.frame()
density_SCD <- kde2d(SCD_feature_coord$x, SCD_feature_coord$y)
density_SCD_df <- kde2d(SCD_feature_coord$x, SCD_feature_coord$y) %>% as.data.frame()
...
```{r angle_calc}
results_matrix <- as.data.frame(matrix(NA, ncol = 1, nrow = 33))
...
for (feature_item in both_list$feature) {
  ...
  points <- matrix(0, ncol = 3, nrow = 2)
  rownames(points) <- c("x", "y")
  colnames(points) <- c("start", "lld", "scd")
  
  # assign the mean of the coordinate points from density output
  points[1,2] <- mean(density_LLD_df$x, trim = 0, na.rm = FALSE)
  points[2,2] <- mean(density_LLD_df$y, trim = 0, na.rm = FALSE)
  points[1,3] <- mean(density_SCD_df$x, trim = 0, na.rm = FALSE)
  points[2,3] <- mean(density_SCD_df$y, trim = 0, na.rm = FALSE)
  
  # calculate distances between the points
  distance_start_lld = crossdist(points[1,1], points[2,1], points[1,2], points[2,2]) 
  distance_start_scd = crossdist(points[1,1], points[2,1], points[1,3], points[2,3]) 
  distance_lld_scd = crossdist(points[1,2], points[2,2], points[1,3], points[2,3]) 
  
  # (distance_lld_scd)^2 = (distance_start_scd)^2 + (distance_start_lld)^2 - 2 (distance_start_scd * distance_start_lld) * cos^-1
  # cos^-1 is acos()
  
  number_calc <- ((distance_start_scd^2) + (distance_start_lld^2) - (distance_lld_scd^2)) / (2*distance_start_scd*distance_start_lld)
...
# sort the results  
results <- results_matrix %>% arrange(desc(distance))
```
